...
process kraken2 {
    ...
    debug true
    pod label: "name", value: "gatekeeper_pipeline:kraken2"
    ...
    if [ \$num_reads -ge ${min_num_reads} ]
      then
           ...
           python3 ${moduleDir}/../lib/filter_kraken_taxa.py\
                -k  ${kraken2_json}\
                -r ${kraken2_report} \
                -s1 $fq1\
                -s2 $fq2\
                -o1  ${kraken2_filtered_reads_1}\
                -o2 ${kraken2_filtered_reads_2}\
                --taxids ${relevant_taxids} --include-children
